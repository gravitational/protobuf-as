...
export class Person {
  ...
  static decodeDataView(view: DataView): Person {
    ...
    while (!decoder.eof()) {
      ...
      switch (number) {
        ...
        case 2: {
          obj.Surname = decoder.string();
          break;
        }
        case 3: {
          obj.Age = decoder.int32();
          break;
        }
        case 4: {
          obj.Weight = decoder.float();
          break;
        }
        case 5: {
  ...
  public size(): u32 {
    ...
    size +=
      this.Surname.length > 0
        ? 1 + __proto.Sizer.varint64(this.Surname.length) + this.Surname.length
        : 0;
    size += this.Age == 0 ? 0 : 1 + __proto.Sizer.int32(this.Age);
    size += this.Weight == 0 ? 0 : 1 + 4;

    if (this.Id != null) {
      const f: Id = this.Id as Id;
  ...
  encodeU8Array(
    ...
    if (this.Surname.length > 0) {
      encoder.uint32(0x12);
      encoder.uint32(this.Surname.length);
      encoder.string(this.Surname);
    }
    if (this.Age != 0) {
      encoder.uint32(0x18);
      encoder.int32(this.Age);
    }
    if (this.Weight != 0) {
      encoder.uint32(0x25);
      encoder.float(this.Weight);
    }

    if (this.Id != null) {
      const f = this.Id as Id;
...
export class Id {
  ...
  static decodeDataView(view: DataView): Id {
    ...
    while (!decoder.eof()) {
      ...
      switch (number) {
        case 1: {
          obj.Number = decoder.string();
          break;
        }
        case 2: {
          obj.Serial = decoder.string();
          break;
        }
  ...
  public size(): u32 {
    ...
    size +=
      this.Number.length > 0
        ? 1 + __proto.Sizer.varint64(this.Number.length) + this.Number.length
        : 0;
    size +=
      this.Serial.length > 0
        ? 1 + __proto.Sizer.varint64(this.Serial.length) + this.Serial.length
        : 0;
  ...
  encodeU8Array(
    ...
    if (this.Number.length > 0) {
      encoder.uint32(0xa);
      encoder.uint32(this.Number.length);
      encoder.string(this.Number);
    }
    if (this.Serial.length > 0) {
      encoder.uint32(0x12);
      encoder.uint32(this.Serial.length);
      encoder.string(this.Serial);
    }
